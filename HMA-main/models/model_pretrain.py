...
class MUMC_Pretrain(MUMCBASE):
    ...
    def forward(self, image, text, region_boxes=None, region_texts=None, alpha=0.):
        with torch.no_grad():
            self.temp.clamp_(0.001, 0.5)


        image_embeds, image_atts, image_feat = self.get_vision_embeds(image)
        text = self.tokenizer(text, padding='longest', truncation=True, max_length=100, return_tensors="pt").to(image.device)
        text_embeds, text_feat = self.get_text_embeds(text)


        region_visual_feats = self.get_region_feats(image, region_boxes)
        region_text_feats = self.get_region_text_feats(region_texts, device=image.device)
        loss_rtc = self.get_region_itc_loss(region_visual_feats, region_text_feats, device=image.device)
        ...
        return loss_ita, loss_mlm, loss_itm, loss_rtc
